...
description: >
  ...
  accent, the three-step gray text ladder, Space Grotesk (display/numerals/chrome) + Space Grotesk (body),
...
typography:
  # — reading ramp (Space Grotesk body + Space Grotesk chrome) —
  body:    { fontFamily: "Inter", cqw: 0.95, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Inter", cqw: 0.9, weight: 600, tracking: "0.14em", upper: true, color: "primary" }
...
This is the **blue-professional** preset remixed onto the blastudios brand, then hand-corrected.
The YAML frontmatter above (colors · typography · components) is **normative and already correct —
use it verbatim.** The prose below is the ORIGINAL preset's intent; read it THROUGH the frontmatter
and apply these four overrides, which win over anything the prose says:

1. **The ground is BLACK, not cream.** `{colors.bg}` is `#000000`. Everywhere the prose says "warm
   cream canvas" or "cream ground", read **pure black**. Headlines are `{colors.text}` `#F5F5F7`
   (near-white), NOT near-black. This is a dark system; the premium signal is black + one blue.
2. **Two faces, fixed roles.** **Space Grotesk 600–700** for display, headlines and numerals;
   **Inter 400–600** for body copy and all uppercase chrome. This is blastudios' real pairing.
3. **The colour law is narrative, and it is load-bearing.** `{colors.warn}` `#F4BA95` belongs to the
   PAIN frames only (01–03): it colours the wasted hours. `{colors.primary}` `#2563EB` belongs to
   the SOLUTION frames only (04–06). **The two never share a frame.** The cut at frame 04 is where
   peach dies and blue takes over — that switch IS the argument.
4. **Cards are white-tinted, not blue-tinted.** `{colors.card-bg}` is `rgba(255,255,255,0.04)` with
   a `rgba(255,255,255,0.08)` border — this is blastudios' real `.glass-panel` from its site. Still
   no shadows: on black, the tint alone does the lift.

Everything else from the preset holds: single accent, no second brand colour, pill chrome, rounded
corners, tinted depth without shadow, the persistent progress bar.
